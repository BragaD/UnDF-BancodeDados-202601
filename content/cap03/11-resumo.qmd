...
## Dica
Use `WHERE sigla_curso = 'PED'` e `ORDER BY nome ASC`.
:::
...
::: {.callout-tip collapse="true"}
## Dica
Use `SUM(carga_horaria)` e `AVG(carga_horaria)` com `WHERE sigla_curso = 'ENS'`.
:::
...
::: {.callout-tip collapse="true"}
## Dica
Use `IN` com uma subconsulta que seleciona `escola` das disciplinas via a relação `curso` para encontrar as escolas relevantes. Ou junte `professor` com `curso` e `disciplina` usando `JOIN`.
:::
