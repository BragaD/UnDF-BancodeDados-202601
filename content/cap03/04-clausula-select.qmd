...
SELECT cod_disciplina || '/' || CAST(ano AS TEXT) || '-' || CAST(semestre AS TEXT) || turma AS id_turma,
       matricula_prof,
       sala
FROM   ministra
ORDER BY id_turma;
...
--| connection: con
--| eval: true
-- DISTINCT com múltiplas colunas: combinações únicas de escola e ch_semanal
SELECT DISTINCT escola, ch_semanal
FROM   professor
ORDER BY escola, ch_semanal;
```

::: {.callout-note}
`DISTINCT` com múltiplas colunas elimina duplicatas de **combinações** — não de cada coluna individualmente. A consulta acima retorna cada par (escola, ch_semanal) único.
:::
