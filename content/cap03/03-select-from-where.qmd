...
WHERE  p.escola  = e.escola
  AND  e.centro  = c.centro
ORDER BY c.centro, p.nome;
```
...
--| connection: con
--| eval: true
-- Alunos cujo ano de ingresso coincide com o semestre da disciplina que cursaram
-- (caso limite: comparação entre atributos de tabelas diferentes)
SELECT DISTINCT a.nome, a.ano_ingresso, md.cod_disciplina
FROM   aluno a, matricula_disciplina md
WHERE  a.matricula      = md.cod_matricula
